...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        {loading && size !== "icon" && (<Loader className="animate-spin" size={loadingSize[size || "default"]} />)}
        {loading && size === "icon" ? (<Loader className="animate-spin" size={loadingSize["icon"]} />) : children}
      </Comp>
    )
  }
)
